    ...
    Statement = [
      {
        ...
        Resource = [ 
          "arn:aws:rds-db:${ var.aws_region }:${ data.aws_caller_identity.current.account_id }:dbuser:${ var.rds_dbi_resource_id }/${ var.db_runner }",
          "arn:aws:rds-db:${ var.aws_region }:${ data.aws_caller_identity.current.account_id }:dbuser:${ var.rds_dbi_resource_id }/${ var.db_migrator }",
        ]
      },
      {
        Effect = "Allow"
